...
class Main_:
	...
	def __check_update_(self, version_):
		try:
			version = requests.get("https://github.com/AMIR-GURUNG/Public/blob/main/date/version.txt").text.strip()
		except requests.exceptions.ConnectionError:
			kotak("# UPS... SEPERTINYA JARINGAN ANDA TERPUTUS",M,C)
			os.sys.exit()
		if version == version_:
	...
	def __check_status_(self, mainx):
		try:
			mainz = requests.get("https://github.com/AMIR-GURUNG/Public/blob/main/date/status.txt").text.strip()
		except requests.exceptions.ConnectionError:
			kotak("# UPS... SEPERTINYA JARINGAN ANDA TERPUTUS",M,C)
			os.sys.exit()
		if mainx == mainz:
